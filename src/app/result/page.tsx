...
export default function ResultPage() {
  ...
  // iTunes 30초 미리듣기는 <PreviewPlayer> 컴포넌트로 추출됨 (src/components/PreviewPlayer.tsx)
  ...
  return (
    ...
    >
      ...
      <div className="px-5 pb-2">
        ...
        {/* 미리듣기 플레이어 — iTunes URL 있으면 자동 표시. 컴포넌트가 fetch·재생·tracking 전담. */}
        {result?.song && (() => {
          const songName = result.song.includes(" - ") ? result.song.split(" - ")[0] : result.song;
          const artistName = result.song.includes(" - ") ? result.song.split(" - ").slice(1).join(" - ") : "";
          if (!songName || !artistName) return null;
          return <PreviewPlayer song={songName} artist={artistName} pageContext="result" />;
        })()}
