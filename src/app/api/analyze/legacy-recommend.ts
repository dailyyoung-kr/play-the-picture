...
export async function legacyRecommend(
  ...
  const listeningStyle = (body.listeningStyle as string) ?? "";
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    console.log(`[legacy] 시도 ${attempt + 1}/${maxAttempts} | 장르: ${genre}`);
    ...
    try {
      ...
      console.log(`[legacy] JSON 파싱 실패, 재시도`);
      continue;
    }
    ...
    console.log(`[legacy] 추천: "${songField}" | 검색: "${searchSong} - ${searchArtist}"`);
    ...
    if (spotifyToken) {
      ...
      if (found && !found.rateLimited && found.trackId) {
        ...
        finalResult = found.trackName && found.trackArtist && !hasKorean
          ...
          : result;
        console.log(`[legacy] ✓ Spotify 검증 성공: ${spotifyTrackId}`);
        break;
      } else if (found?.rateLimited) {
        console.log(`[legacy] 429 감지 → 즉시 반환`);
        finalResult = result;
        break;
      } else {
        console.log(`[legacy] ✗ Spotify에서 찾지 못함 (시도 ${attempt + 1}/${maxAttempts})`);
        if (attempt === maxAttempts - 1) finalResult = result;
        continue;
      }
    } else {
      finalResult = result;
  ...
  if (!finalResult) {
    if (lastValidResult) {
      console.log(`[legacy] finalResult null → lastValidResult로 fallback`);
  ...
  console.log("[legacy] 완료:", finalResult.song, "| id:", spotifyTrackId);
