...
### 코드 (완료)
- [x] RN: 오늘의 발견 풀스택 (commit `e58fe0f`) + stale fix (`eeaa8de`) → push 완료
- [x] 웹: 오늘의 발견 풀스택 (commit `85a1e78`) + stale fix (`fa99076`) → push 완료
...
### 빌드 (진행 중)
- [x] EAS production build 등록 (Build #9, ID `badb5b74-...`)
- [x] TestFlight 자동 제출 등록 (`74cdfabb-...`)
- [ ] 빌드 완료 대기 (15~25분)
- [ ] TestFlight 처리 대기 (5~15분)

### App Store Connect (사용자 직접)
- [ ] Privacy Nutrition Label 보완 (위 §12 참조)
- [ ] What's New 작성
- [ ] 새 빌드 (Build 9) 선택 + 심사 제출
- [ ] (선택) 스크린샷에 발견 화면 1~3컷 추가

### TestFlight 실기기 검증
- [ ] Google 로그인 정상 (시뮬레이터에선 SDK 제약으로 깨졌던 부분)
- [ ] 오늘의 발견 탭 진입 → 캐러셀 정상
- [ ] 자세히 보기 → 상세 페이지 + 미리듣기
- [ ] 별·북마크 저장 → 컬렉션 반영
- [ ] 비로그인 상태에서 발견 탭 → 로그인 모달 즉시
- [ ] entries 0건 로그인 상태에서 발견 탭 → 빈 카드 안내 UI
