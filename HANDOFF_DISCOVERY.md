...
### 미해결·후속 작업
- ✅ RN 앱 실기기 검증 → **1.0.2 EAS 빌드 진행 중 (Build #9, TestFlight 자동 제출)**
- 메인 페이지 진입 link (랜딩에서 발견 페이지로 들어가는 별도 CTA, 사용자 요청 시)
- ✅ production 배포 → **약관·발견 페이지 모두 main push 완료, Vercel 자동 배포**
...
### 즉시 처리 가능
- [x] ~~production 배포~~ → 2026-05-30 완료 (web + RN 1.0.2 빌드 등록)
- [ ] **cron 등록** — 매일 KST 0시 자동 카드 사전 생성 (현재는 첫 방문자가 lazy trigger, footer 문구 "오후 12시"와 실제 갱신 시점 불일치)
- [x] ~~RN 앱 2~7단계~~ → 1~6단계 모두 완료
- [ ] **App Store Connect Privacy Nutrition Label 보완** — 현재 누락된 항목:
  - 🔴 User Content > **Other User Content** (discovery_saves, vibeType, vibe_description 등) — 1.0.2 신기능 직접 관련, 진행 중
  - 🔴 Identifiers > **Device ID** (모든 사용자 익명 ID) — 누락 시 거절 위험
  - 🔴 Usage Data > **Product Interaction** (GA + Meta + 자체 로그)
  - ⚠️ 사진 항목의 "사용자 신원 연결 여부" 확인 권장 (저장 시 user_id 매핑 → 연결됨)
- [ ] **What's New 작성** (1.0.2 App Store Connect)
- [ ] **footer "오후 12시" 문구 정확화** — cron 도입 안 했으면 "매일 자정 갱신" 또는 "매일 새로운 아티스트가 도착해요" 권장
...
- [ ] iOS 시뮬레이터 Google 로그인 제약 — 실기기에선 정상 가능성. TestFlight 검증 필수
- [ ] 컬렉션 규모 ≥100개 도달 시 페이지네이션·DB 인덱스 검토 (현재 limit 없음, ORDER BY saved_at DESC만)
...
- 활성 사용자도 익명 로그인은 통과 (`is_anonymous=true` → userId 있음). 완전 정식 회원만 받고 싶으면 별도 분기 필요

## 15. 1.0.2 출시 체크리스트 (2026-05-31 기준)

### 코드 (완료)
- [x] RN: 오늘의 발견 풀스택 (`e58fe0f`) + stale fix (`eeaa8de`) + 상세 별·컬렉션 삭제 (`00762c7`) → push 완료
- [x] 웹: 오늘의 발견 풀스택 (`85a1e78`) + stale fix (`fa99076`) + 상세 별·컬렉션 삭제·중복 제외 (`2406366`) → push 완료
- [x] 약관 2개 업데이트 (시행일 5/30)
- [x] Apple Music 어트리뷰션 추가
- [x] Vercel 자동 배포 + production 환경변수 (Apple Music JWT) 등록
- [x] Supabase migration_020·021·cleanup 적용

### 빌드·심사 (완료)
- [x] EAS production build #9 등록 (`badb5b74-...`) — 1차 시도
- [x] EAS production build #10 등록 (`cfcc0b0f-...`) — 상세 별·삭제·중복 제외 포함
- [x] TestFlight 자동 제출
- [x] App Store Connect Privacy Nutrition Label 보완 (Other User Content 추가)
- [x] What's New 입력 (오늘의 발견 + 내 컬렉션 + Google 로그인)
- [x] **App Store 1.0.2 심사 제출 완료** (수동 출시 옵션)

### 심사 후 (대기)
- [ ] Apple 심사 결과 (평균 24~48시간)
- [ ] 통과 시 "이 버전 출시" 클릭 → App Store 게시
- [ ] 거절 시 사유 확인 → 1.0.3 hotfix
...
| 날짜 | 내용 |
|------|------|
| 2026-05-31 | **App Store 1.0.2 심사 제출 완료** (Build #10). What's New: "오늘의 발견 신기능 + 내 컬렉션 + Google 로그인 안정성 개선". 수동 출시 옵션, Privacy Nutrition Label 보완 (Other User Content 추가) 완료. |
| 2026-05-31 | **중복 추천 제외 (옵션 2)** — `src/lib/discovery-engine.ts`. `getSavedArtistIds` 헬퍼 추가 (cache_key 기준 discovery_saves의 artist apple_id Set 조회). `resolveArtistPair`에 `excludeIds` parameter — 1차: 이미 저장한 아티스트 제외, 2차 폴백: 시드 풀 부족 시 excludeIds 무시. 콜드 풀(`forceColdStart=true`)은 영향 X. 효과: 매일 새로운 아티스트 보장 + 시나리오 B(실수 삭제) 위험 ↓. |
| 2026-05-31 | **컬렉션 카드 삭제 기능 (RN + 웹)**. 아티스트 카드 artwork 우상단 + 곡 카드 우측 저장일 옆에 `Trash2` 휴지통 28×28 원형 버튼. optimistic UI + 백그라운드 `toggleSave` (toggle이라 다음 호출이 unsave). identity state 캐싱. 실패 시 RN은 `load()` 재호출 / 웹은 console.error. |
| 2026-05-31 | **상세 페이지 artwork 좌상단 별 저장 (RN + 웹)**. 캐러셀의 별과 동일 디자인 (Star 28px + drop-shadow + isSaved 시 노란 `#FFD23F` fill). `savedArtists` state + `handleSaveArtist` (optimistic + login gate). RN [idx].tsx의 DetailView, 웹 page.tsx의 DetailView 모두 적용. commit RN `00762c7`, 웹 `2406366`. |
| 2026-05-31 | **Vercel production 환경변수 추가** — Apple Music JWT (`APPLE_MUSIC_TEAM_ID`, `APPLE_MUSIC_KEY_ID`, `APPLE_MUSIC_KEY`). 초기 ".p8 형식 깨짐(ES256 에러)" 발생 → 재입력 (multiline 그대로 paste, BEGIN/END 마커 포함) 후 해결. production discovery API 정상 동작 확인. |
| 2026-05-31 | **1.0.2 EAS production 빌드 등록**. Build ID `badb5b74-098a-4e88-b06b-682ee0b77254`, Build Number 9 (autoIncrement). `--auto-submit` 으로 TestFlight 자동 제출 등록 (Submission ID `74cdfabb-9b9b-4c2d-81c6-2ac15498f5c8`). 빌드 클라우드에서 진행 중. |
| 2026-05-31 | **stale 카드 갱신 버그 수정 (RN + 웹)**. 자정 넘어 cache_key date가 바뀌어도 캐러셀 state가 old인 문제 (자세히 보기는 새 fetch라 카드 불일치). `todayKstStr()` 헬퍼 + `fetchedDate` state 도입. RN은 `useFocusEffect`에서 date 비교 → 다르면 `loadData` 재호출. 웹은 `visibilitychange` 이벤트로 동일 처리. commit: RN `eeaa8de`, 웹 `fa99076`. |
| 2026-05-30 | **약관 업데이트 (1.0.2 출시 준비)**. `/privacy`: §1에 "발견 카드 저장 데이터" 추가, §2 이용 목적에 "오늘의 발견 카드 개인화 추천", §3 보유 기간 추가, §4 위탁에 "Apple Music" 보강. `/terms`: 제1조 ‘서비스’ 정의에 "매일 큐레이션된 아티스트를 소개하는 ‘오늘의 발견’ 기능" 추가 (숫자 명시 X). 시행일 2026-05-30. |
| 2026-05-30 | **Apple Music API 어트리뷰션 추가 (RN + 웹)**. 발견 페이지 footer "Powered by Apple Music" (활성 사용자 카드 노출 시). 상세 페이지 추천곡 헤더 아래 "Powered by Apple Music" (Apple Music API에서 곡 데이터·preview 가져옴). `/result`·`/journal`의 PreviewPlayer는 iTunes Search API 출처라 기존 "미리듣기 제공: iTunes" 유지. |
| 2026-05-30 | **컬렉션 빈 상태 안내문구 변경 (RN + 웹)**. "아직 저장한 X" → **"아직 발견한 X"** (오늘의 발견 어휘로 통일). 아티스트 빈 안내: "좌상단 별" → **"별모양"** (위치 명시 제거, 모양 자체로 직관화). 곡 안내는 그대로 (북마크). |
| 2026-05-30 | **EmptyOnboard UI 최종 정제 (여러 차례 반복 후 Option C 채택)**. <br/>**제거**: 헤더 ✨ Sparkles 아이콘, "노래 추천 받기" 버튼 (하단 네비와 중복으로 인지 부담), 헤더 아래 안내 텍스트 영역. <br/>**최종 구조**: 빈 카드 안에 모든 안내 통합 → 🔎 Search 아이콘 (44px, drop) + **보라 라벨** "아직 추천 기록이 없어요" (14px·700·PURPLE_MAIN·letter-spacing 0.3) + **설명** "사진으로 곡을 추천 받아 저장하면\n매일 내 취향에 맞는 아티스트 2명을 소개해드려요" (13px·흐림·center). 카드 frame은 활성 사용자 CarouselSlide와 동일 4:5 portrait + 흐릿한 점 인디케이터 (●·) 미리보기. 헤더는 "오늘의 발견" 텍스트만 (페이지 식별 단순화). |
